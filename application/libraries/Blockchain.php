...
class Blockchain{
	...
	public function __construct($config)
	{
		...
		if ($this->_exec('') === NULL) {
			...
			echo "<script>console.log( 'Debug Objects: Blockchain Wallet: Unable to connect to Blockchain Wallet Service on: ". $this->base_url .":".$this->port."' );</script>";
		}
	}
	 ...
	 * @return array API's response
	 */
	public function send($to, $amount, $from = NULL, $fee = NULL, $guid = null)
	{
		// Build parameters
		$parameters = [
		...
		$get_guid 	= ( $guid != null )? $guid : $this->guid;

		// Execute
		return $this->_exec('merchant/'.urlencode( $get_guid ).'/payment', $parameters);
	}
	 ...
	 * @return array API's response
	 */
	public function address_balance($address, $guid = null)
	{
		// Build parameters
		$parameters = [
			'password' => $this->main_password,
		...
		// Execute
		return $this->_exec('merchant/'.urlencode( $get_guid ).'/address_balance', $parameters);
	}
	...
	public function new_address($label = NULL)
	{
		...
		// Execute
		return $this->_exec('merchant/'.urlencode($this->guid).'/accounts/create', $parameters);*/
